...
class GlobalExceptionHandler {
    ...
    fun handleRatingServiceException(ex: RatingServiceException, request: HttpServletRequest): ResponseEntity<ProblemDetail> {
        log.warn("Rating service error: ${ex.message}")
        val problem = ProblemDetail.forStatusAndDetail(HttpStatus.BAD_REQUEST, ex.message)
        problem.title = "Rating Service Error"
        problem.instance = URI.create(request.requestURI)
        problem.setProperty("timestamp", Instant.now().toString())
        return ResponseEntity(problem, HttpStatus.BAD_REQUEST)
    }
